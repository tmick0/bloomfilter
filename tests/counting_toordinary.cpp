...
int main(int argc, char *argv[]){
    ...
    bloom::OrdinaryBloomFilter<std::string> bf_2 = bf.ToOrdinaryBloomFilter();
    
    if(bf_2.GetNumHashes() != bf.GetNumHashes()){
        std::cout << "Error: Converted BF disagrees on numHashes." << std::endl;
        return 1;
    }
    
    if(bf_2.GetNumBits() != bf.GetNumBits()){
        std::cout << "Error: Converted BF disagrees on numBits." << std::endl;
        return 1;
    }
