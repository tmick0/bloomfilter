...
int main(int argc, char *argv[]){
    ...
    bloom::OrdinaryBloomFilter<std::string> bf_2 = bf.Compress();
    
    if(bf_2.GetNumHashes() != bf.GetNumHashes()){
        std::cout << "Error: Compressed BF disagrees on numHashes." << std::endl;
        return 1;
    }
    
    if(bf_2.GetNumBits() != bf.GetNumBits() / 2){
        std::cout << "Error: Compressed BF disagrees has wrong numBits." << std::endl;
        return 1;
    }
